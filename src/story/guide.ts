...
export const GUIDE: GuideStep[] = [
  {
    ...
    todo: ['Write your premise: who, what happens, what\'s at stake', 'Open the "Tone & feel" tab and set the sliders', 'Describe how it should feel, in your own words', 'Add the books and authors you love, then turn them into a style guide'],
  ...
  },
  {
    ...
    todo: ['Just write. It saves by itself', 'Stuck on a scene? In your editor, choose "Write" and type or speak your rough notes', 'Blank page? On an empty chapter, "Draft this chapter for me" writes a first version from your plan. Then rewrite it in your own words', 'Try the Talk button to speak instead of typing'],
  ...
  {
    id: 'gripping',
    title: 'Make it unputdownable',
    page: 'lab',
    why: 'A publishable thriller has to be gripping, not just correct. The Story Lab reads your book the way readers will, and shows where it sags and when readers crack the mystery.',
    todo: ['Run the Page-turner analysis and look at the red "put-down" chapters', 'Run the Solvability test to see when readers guess the culprit', 'Try the stress tests: the killer\'s counter-move, set pieces, "Would readers root for her?"'],
    check: (p) => !!p.lab.bookAnalysis && !!p.lab.solvability,
  },
  {
    id: 'ending',
    title: 'Your ending',
  ...
  {
    id: 'opening',
    title: 'Perfect your opening pages',
    page: 'lab',
    why: 'Agents decide on the first page, often the first paragraph. Your opening has to promise the whole book.',
    todo: ['Story Lab → Opening pages: run "The agent\'s desk"', 'Try the first-line workshop', 'Ask the reader panel about Chapter 1'],
    check: () => false,
    ask: { action: 'firstPages', label: 'The agent\'s desk' },
  },
  {
    id: 'polish',
    title: 'Polish like a professional',
    page: 'polish',
    why: 'Clean, consistent pages tell an agent you\'re a professional. Most of these checks are free and instant.',
    todo: ['Read the Manuscript health report (length, chapter lengths, habit words)', 'Fill in your style sheet and fix any inconsistencies', 'Proofread each chapter, and run the permissions check'],
    check: () => false,
  },
  {
    id: 'finish',
    title: 'Get it out into the world',
    page: 'publish',
    why: 'A great book still needs a great pitch. Human readers first, then a query, a synopsis, and the right agents.',
    todo: ['Send chapters to 3–6 human readers (Publish → Human readers)', 'Draft your query letter and synopsis, then test the query on the agent panel', 'Find agents, track submissions, and download your submission package'],
    check: (p) => p.publishing.agents.some((a) => a.status !== 'researching'),
    ask: { action: 'queryBuilder', label: 'Draft my query letter' },
  },
  {
    id: 'series',
    title: 'Think about a series',
    page: 'series',
    why: 'A book that stands alone but could grow into a series is especially attractive to agents, and when you\'re ready, the next book starts from everything you\'ve built.',
    todo: ['Run the Series potential check', 'Plant a sequel seed or two', 'When Book 1 is done, click "Start the sequel"'],
    check: (p) => !!p.series.seeds.trim() || !!p.series.arc.trim(),
  },
];
